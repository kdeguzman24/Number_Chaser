...
class AddHard : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.add_hard)
    ...
    private fun startGame() {
        ...
        if (currentRound > 0) {
            ...
            for (i in 1..5) {
                var nextNumber: Int
                do {
                    nextNumber = Random.nextInt(5, 16)
                } while (sequence.isNotEmpty() && (nextNumber == lastNumber || isConsecutive(sequence.last(), nextNumber)))
    ...
    private fun displayNextNumber() {
        if (currentIndex < sequence.size) {
            ...
            Handler().postDelayed({
                currentIndex++
                displayNextNumber()
            }, 1000)                        //1000               // eto yung deley sa pag-appear nung mga numbers
        } else {
