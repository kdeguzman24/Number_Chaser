...
class AddEasy : AppCompatActivity() {


    private val totalRounds = 3                                             //  <== Total nung Rounds na meron sa kada level
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.add_easy)
    ...
    private fun startGame() {
        ...
        // Reset the remaining time for each round
        ...
        if (currentRound > 0) {
            ...
            var lastNumber = -1 // initialize with a value that is not possible in your sequence

            for (i in 1..5) {
                var nextNumber: Int
                do {
                    nextNumber = Random.nextInt(1, 6)
            ...
            // Delay before showing the EditText and starting the 5-second countdown
            Handler().postDelayed({
                // Move the visibility setting here, after the sequence is displayed
                answerEditText.visibility = View.VISIBLE
                submitButton.visibility = View.VISIBLE
                startEditTextTimer() // Start the 5-second countdown for EditText
            }, 1000 * sequence.size.toLong() + 10000) // Adjusted delay for better synchronization
        } else {
    ...
    private fun displayNextNumber() {
        if (currentIndex < sequence.size) {
            ...
            Handler().postDelayed(
                ...
                3000
            )                        //1000               // eto yung deley sa pag-appear nung mga numbers
        } else {
    ...
    private fun checkAnswer() {
        ...
        val correctAnswer = sequence.sum()
        answerEditText.visibility = View.GONE
    ...
    private fun restartGame() {
        ...
        // Add null checks and cancel timers only if they are not null
